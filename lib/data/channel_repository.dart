...
//  static const String INVITINGUSER = "inviting_user";
  static const String INVITING_MEMBER = "inviting_member";
...
//  final Firestore _firestore;
  ...
  Stream<List<Channel>> getChannelsStream(String groupId, int memberId) {
//    return _firestore
...
//        return await documentToChannel(groupId, memberId, document);
//      }));
//    });
  }

  Stream<Channel> getStreamForChannel(
      String groupId, String channelId, int userId) {
//    return _firestore
...
//    });
  ...
  Future<void> markChannelRead(
      String groupId, String channelId, int userId) async {
...
//    return channel.rebuild((c) => c..users.addAll(users));
  ...
  Future<Channel> createChannel(
      String groupId, Channel channel, List<int> members, int authorUid) async {
...
//        .updateData({RSVP_FIELD: RSVPHelper.stringOf(rsvp)});
  ...
  static toChannelUserMap(ChannelMember member, {bool isInvite = false}) {
    return {
      MEMBERID: member.id,
      RSVP_FIELD: RSVPHelper.stringOf(member.rsvp),
      INVITATION: isInvite,
    };
  }
  ...
  static toChannelUserInviteMap({
    ...
    channelUserInviteMap.addAll({
      METADATA: {
        ...
        INVITING_MEMBER: invitingUsername,
        GROUPNAME: groupName,
      }
    });
  ...
  static Map<String, dynamic> toMap(Channel channel, List<String> members) {
    return {
      ...
      VENUE: channel.venue,
//      START_DATE: _formatToTimestamp(channel),
      START_DATE: channel.startDate.millisecondsSinceEpoch,
