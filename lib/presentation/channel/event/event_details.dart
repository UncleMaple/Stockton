...
import 'package:stockton/presentation/channel/event/event_details_view_model.dart';
...
import 'package:stockton/presentation/member/member_item.dart';
...
class _EventDetailsWidgetState extends State<_EventDetailsWidget>
  ...
  TabBar _buildTabBar() {
    return TabBar(
      ...
      tabs: <Widget>[
        _buildTab(StocktonLocalizations
            .of(context)
            .eventDetails),
        _buildTab(StocktonLocalizations
            .of(context)
  ...
  Widget _buildMembersCount() {
    return Row(
      children: <Widget>[
        ...
        Text(
          StocktonLocalizations.of(context)
  ...
  Widget _buildUserRsvp() {
    return Visibility(
      visible: widget.vm.memberRsvp != RSVP.UNSET,
      child: Row(
        children: <Widget>[
          Padding(
            padding: const EdgeInsets.all(AppTheme.appMargin),
          ...
          Padding(
            ...
            child: RsvpIcon(rsvp: widget.vm.memberRsvp),
          ),
          Visibility(
            visible: widget.vm.canChangeRsvp,
            child: FlatButton(
              child: Text(
                StocktonLocalizations.of(context).eventRsvpChange,
                style: AppTheme.buttonTextStyle,
              ),
              onPressed: () {
    ...
    return ListView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        if (index < widget.vm.members.length) {
          ...
          return MemberItem(
            member: member,
            rsvp: widget.vm.rsvpStatus[member.id],
            isYou: member.id == widget.vm.member.id,
            isHost: member.id == widget.vm.channel.authorId,
          );
        } else {
          return Padding(
            padding: const EdgeInsets.all(AppTheme.appMargin),
            child: RoundButton(
              text: StocktonLocalizations
                  .of(context)
                  .channelInviteButton,
              onTap: () {
    ...
    return Visibility(
      ...
      child: Padding(
        ...
        child: Center(
          child: FlatButton(
            child: Text(
              StocktonLocalizations.of(context).eventLeave,
              style: AppTheme.buttonTextStyle,
            ),
            onPressed: () {
  ...
  _showLeaveChannelDialog(BuildContext context) {
    showPlatformDialog(
      ...
      builder: (_) => PlatformAlertDialog(
        title: Text(StocktonLocalizations.of(context).channelLeaveAlertTitle),
        content:
        Text(StocktonLocalizations.of(context).channelLeaveAlertMessage),
        actions: <Widget>[
          PlatformDialogAction(
              child: Text(StocktonLocalizations.of(context).yes),
              onPressed: () {
          ...
          PlatformDialogAction(
              child: Text(StocktonLocalizations.of(context).cancel),
              onPressed: () {
                Navigator.pop(context);
              }),
        ],
